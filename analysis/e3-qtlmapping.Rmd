...
- Install R packages, `MatrixEQTL`, `dplyr`.
...
So these two models don't catch any significant effect of explored SNPs and gene expression levels. In fact it is very hard to catch such effects by "manually" exploring real-world genomic data, usually containing tens or hundreds of thousands SNPs and hundreds or thousands gene expressions for large number of samples. Many softwares have been developed to perform eQTL mapping efficiently for thousands of snp-gene pairs, such as [MatrixEQTL](http://www.bios.unc.edu/research/genomic_software/Matrix_eQTL/), [tensorQTL](https://github.com/broadinstitute/tensorqtl) (GPU-accelerated, ~200x faster than CPU-based tools), and [QTLtools](https://qtltools.github.io/qtltools/). Note: fastQTL is no longer actively maintained and has been largely superseded by tensorQTL and QTLtools.
...
`MatrixEQTL` does this for you automatically and returns the corrected p-value as a false discovery rate (FDR). Common thresholds on FDR are 5% or 10%. Tools like tensorQTL and QTLtools use a permutation-based strategy to obtain gene-level p-values, followed by FDR correction across genes.
...
y_range = range(plot_data[,"gene_expr"])
  ...
  scientific=TRUE, digits=2)), col = "darkorange")
```
