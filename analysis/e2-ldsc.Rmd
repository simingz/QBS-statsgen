...
There are two options for installing LDSC. If you haven't installed Anaconda, please check [how to install anaconda](https://cloufield.github.io/GWASTutorial/80_anaconda/) first.

#### Option 1: Original LDSC (Python 2)

The original LDSC ([https://github.com/bulik/ldsc](https://github.com/bulik/ldsc)) requires Python 2, which reached end-of-life in January 2020. It can still be installed via conda using the environment file provided in the repository. Use this option if you are working on a cluster (e.g., Dartmouth Discovery) where a compatible environment is already available.
...
The [`environment.yml`](https://github.com/bulik/ldsc/blob/master/environment.yml) file is included in the repo root. If conda has trouble resolving packages from `bioconda` alone, add `defaults` and `conda-forge` to the channels before running `conda env create`:
...
#### Option 2: Python 3 fork (recommended for local use)

The community-maintained Python 3 fork ([abdenlab/ldsc-python3](https://github.com/abdenlab/ldsc-python3)) works with modern Python and does not require a Python 2 environment.

```{bash eval=F}
mkdir ~/tools && cd ~/tools
git clone https://github.com/abdenlab/ldsc-python3.git
cd ldsc-python3
conda create -n ldsc python=3.11 -y
conda activate ldsc
pip install scipy pandas numpy bitarray
```

**Note:** As of April 2026, the `abdenlab/ldsc-python3` fork has three bugs that require manual patching before use:

1. **gzip read mode** — in `munge_sumstats.py`, change `gzip.open` to open in text mode:
```python
# Change:
openfunc = gzip.open
# To:
...
2. **Deprecated pandas argument** — replace `delim_whitespace=True` with `sep=r"\s+"` in `munge_sumstats.py`, `ldscore/parse.py`, and `make_annot.py`.

3. **Renamed functions** — in `ldsc.py`, update the three function calls to match the new names in `ldscore/sumstats.py`:
```python
# Change:
...
sumstats.estimate_cell_type_specific_heritability(args, log)
```

Regardless of which option you use, replace `python2` with `python` (Option 1) or `python` (Option 2) in all commands below.
